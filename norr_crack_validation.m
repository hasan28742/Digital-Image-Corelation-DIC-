...
%Digital Image corelation(DIC) project validation for ncorr website crack image
%submitted by Hasan-W10389619
%this code will exclude multiple regin from gridpoints 
%this number may be chnged in code by changing counter number 
%this same code would be modifed in step 2 (one secton would be supressed)
%and in step 3(2 line would) to apply for no exclusion consideration
%whole code is in 2d, No data flattening

clc
clearvars

%%%%%%%%%%%Step 1: Collecting file%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%try to read the reference and last current image to matlab

R = imread('ref.TIF');%R=reference image=ref.tif
C = imread('cur_920.TIF');%C=current image=cur_920.tif
...
grid_spaceing=70; %two grid points distance
[rows,columns] = size(R);
...
w=40; % Width-horizontal length-cartesian x
h=40; %height-vertical lenght-cartesian y
